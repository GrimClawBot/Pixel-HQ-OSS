...
| **Oversized projection materialization** | A section or whole overview exceeds its bound | Section counters, arrays, text, and metadata bounds fail closed per section with `PROJECTION_BOUND_EXCEEDED`; only when the complete serialized envelope exceeds `MAX_BYTES` does `serializeOverview()` return `PROJECTION_TOO_LARGE`; the overview HTTP layer passes both reason codes through untranslated |
