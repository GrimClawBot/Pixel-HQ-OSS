...
export class FakeModelASimulatorAdapter {
  get source() { return 'simulator'; }
  get providerContract() { return MODEL_RUNTIME_ADAPTER_CONTRACT; }
  get runtimeId() { return 'pixel.simulator.model-runtime-a'; }
  get modelId() { return 'pixel.fake-model-a.v1'; }

  invoke(providerRequest) {
    const request = snapshotSafePlainData(providerRequest);
    if (!validateModelProviderRequestV1(request).ok) throw new TypeError('Fake Model A requires a valid provider request');
    const output = `Fake Model A summarized ${request.context_items.length} approved context item(s).`;
    return freeze({
